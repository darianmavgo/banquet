...
3. **Displaying Tiny Tables**: For all tiny tables, the UI should automatically display the *contents* of the table inline or expanded within the list, rather than just showing the table name.
   - For terminal/text renderers, the inline preview area MUST format the contents by enclosing each row's comma-separated values in brackets `[...]`, with each row separated by a pipe ` | ` (e.g., `[1, Electronics] | [2, Furniture]`).
   - Conceptually, this exposes the data as if the user ran a `SELECT * FROM table` directly within the list view.
